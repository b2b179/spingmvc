...
<html>
  ...
  <body>
  	<div class="path">
		<a href="">首页</a> » 用户列表 <span>(共<span id="pageTotal">${pageBean.countData}</span>条记录)</span>
	</div>
	<form id="listForm" action="${pageContext.request.contextPath}/users/queryAll" method="get">
		<div class="bar">
			<a href="${pageContext.request.contextPath}/users/addData" class="iconButton"> <span class="addIcon">&nbsp;</span>添加</a>
		...
		<tbody>
			...
			<tr class="grand_tr">
				...
				<td><span style="margin-left: 0px; color: #000000;">${user.userId}</span></td>
				<td>${user.userName}</td>
				<td>${user.age}</td>
				<td>${user.group.groupName}</td>
				<td>${user.email}</td>
				<td><c:if test="${user.image !=null}"><img alt="description" src="${user.image }"></c:if></td>
				<td>${user.description}</td>
				<td>
					<a href="${pageContext.request.contextPath}/users/updateData/${user.userId}">[编辑]</a>
				</td>
			</tr>
			</c:forEach>
			</tbody>
 ...
 <script type="text/javascript">
		$().ready(function() {
			...
			$pageSizeOption.click( function() {
				...
				window.location.href='${pageContext.request.contextPath}/users/queryAll?page='+$('#currentPage').val()+"&pageSize="+$('#pageSize').val()+"&search="+encodeURI($('#searchValue').val(),'UTF-8');
			});
			...
			$.pageSkip = function(pageNumber) {
				if(pageNumber !=null && pageNumber>0){
					...
					$.getJSON(
						'${pageContext.request.contextPath}/users/operactorByAjax',
						...
						function(jsonBean){
							...
							for(var i=0;i<pageBean.data.length;i++){
								var user  = pageBean.data[i];
								var $tr = $("<tr>");
								$tr.appendTo("#listTable");
								$tr.append('<td><input type="checkbox" name="ids" value="'+user.userId+'"></td>');
								$tr.append('<td>'+user.userId+'</td>');
								$tr.append('<td>'+user.userName+'</td>');
								$tr.append('<td>'+user.age+'</td>');
								$tr.append('<td>'+user.group.groupName+'</td>');
								$tr.append('<td>'+user.email+'</td>');
								$tr.append('<td>'+(user.image!=null ? '<img src="'+user.image+'"/>': '')+'</td>');
								$tr.append('<td>'+user.description+'</td>');
								$tr.append("<td><a href='${pageContext.request.contextPath}/users/updateData/"+user.userId+"'>[编辑]</a></td>");
							}
						}
					);
				}
		...
		$deleteButton.click(function(){
			...
			if(confirm("确认要删除吗？ 该组别下的所有参数都会被删除掉...")){
				$.ajax({
					url:"${pageContext.request.contextPath}/users/operactorByAjax",
					type:'POST',
